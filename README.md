...
- “策略猜想”页面持续保存一分钟K线，验证指定分钟前1分钟冲击后的5/15分钟反转表现
...
页面顶部“策略猜想”进入 `HYP-REV-1M-V1` 验证页。系统只保存已经收盘的
一分钟K线；首次启动回填 Gate 当前可返回的最近数据，之后持续写入
PostgreSQL。猜想固定观察北京时间 `00/06/24/30/36/54`，并使用
`12/18/42/48` 作为同一六分钟节奏的对照组。结果只用于研究，不会下单。
...
提醒次数、最后提醒时间和触达时间保存在 PostgreSQL；已触达点位仍保留在
“今日点位记录”中，不会因为退出活跃监控而从页面消失。

系统还会把每个点位作为 `LEVEL-5X5-V1` 长期验证样本写入 PostgreSQL：

- 设置时点位低于最新价：做多
- 设置时点位高于最新价：做空
- 当天 24 点前触达点位才算入场
- 入场后止损和止盈都距离点位 5 美元，跨日继续跟踪到结算
- 胜率为 `胜 ÷（胜 + 负）`；未触发、覆盖和取消的样本不进入分母
- 实时最新价负责低延迟结算，Gate 1 秒 K 线高低点负责补捉瞬时插针
- 同一秒内同时触及止损和止盈、无法确认先后时标为“顺序待复核”，不进入分母

机器人发送“黄金 胜率”可查看累计结果，网页面板也展示胜率和最近逐笔记录。
统计使用 Gate 最新价，不含点差、滑点和手续费，因此是规则命中率，不是实际
净收益率。

网页统计分为三层：

- 累计总览：已结算、胜率、胜负、理论点数、单笔期望、持仓、待触发和待复核
- 每日表现：按北京时间的“点位设置日”查看每日样本、胜率和理论点数
- 逐笔审计：支持方向、状态和日期筛选，并使用游标分页查看完整生命周期

每日范围可以切换最近 7、30、90 或 365 天。累计总览默认包含全部日期；
点击某一天会筛选下方逐笔记录。相邻点位可能由同一波行情同时触发，不能把
每条点位都视为完全独立的统计样本。
